...
def smile(alpha, beta, rho, nu, F, K, time, MKT,
          i):  # F, time and the parameters are scalars, K and MKT are vectors, i is the index for tenor/expiry label
...
def SABR_vol_matrix(alpha, beta, rho, nu, F, K, time,
...
def calibration(starting_par, F, K, time, MKT):
    for i in range(len(F)):
        ...
        res = minimize(objfunc, x0, (F[i], K[i], time[i], MKT[i]), bounds=bnds,
